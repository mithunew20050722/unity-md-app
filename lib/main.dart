...
class UnityMdApp extends StatefulWidget {
  final Widget home;
  const UnityMdApp({super.key, required this.home});
  @override State<UnityMdApp> createState() => _UnityMdAppState();
}

class _UnityMdAppState extends State<UnityMdApp> {
  OverlayEntry? _fabEntry;

  void _insertFab() {
    _fabEntry?.remove();
    _fabEntry = OverlayEntry(builder: (_) => const ControlPanelFab());
    navigatorKey.currentState!.overlay!.insert(_fabEntry!);
  }
...
/// Inserts the FAB once the first route is pushed (Overlay is ready).
class _FabInsertObserver extends NavigatorObserver {
  final VoidCallback onReady;
  bool _done = false;
  _FabInsertObserver(this.onReady);
